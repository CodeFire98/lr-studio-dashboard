...
**Last updated:** 2026-05-26 (**LinkAI composer: paste images directly with Ctrl/Cmd+V.** Third entry point for image attachments alongside the paperclip button and drag-drop. Wired as an `onPaste` handler on the page-variant composer textarea — only fires when the textarea has focus, so it doesn't surprise users pasting anywhere else on the page. Mixed-content paste handling: if the clipboard carries both an image AND plain text (e.g. copying a rich block from Slack/Notion), the image attaches AND the text pastes into the textarea normally; if the clipboard is image-only (the common OS-screenshot-tool path), the no-op default paste is preventDefault'd so nothing weird happens. Browsers name clipboard images "image.png" generically — when multiple images get pasted in one go, all would show the same tray label, so the handler renames them to a timestamped form (`pasted-2026-05-26T14-30-22.png`, `pasted-2026-05-26T14-30-22-2.png`, ...) before they enter the existing `addFiles` pipeline. Same constraints as the paperclip + drag-drop paths: PNG/JPEG/WebP/GIF only, 5 MB per file, 4 attachments per message — all enforced by the shared `addFiles` helper that hasn't changed. Composer hint updated from "drop an image to attach" to "paste or drop an image to attach". Files: `web/src/components/LinkAIPanel.jsx` (+~45 LOC: new `onPaste` callback wired onto the page-variant textarea + hint copy tweak). Pure client change — no schema, migration, env-var, or server work. Drawer variant (which has no paperclip/drag-drop today) is intentionally left alone; PR D will retire the drawer entirely. Sections touched: Recent changes log; `Last updated`.)
