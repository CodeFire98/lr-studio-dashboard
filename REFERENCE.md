...
**Last updated:** 2026-05-08 (sidebar relabels: brand-side "Got ideas?" → "Idea dump"; secondary "Brand team" → "Team" only in brand-owner view (agency-in-brand still says "Brand team" to disambiguate from "L+R Team"). Plus invite-redemption robustness: pre-flight email check before calling `accept_invitation`, with a "Sign out & switch account" CTA on mismatch — token survives so re-sign-in completes the redemption.)
...
Most recent: `0036_post_plan_publications.sql`.
