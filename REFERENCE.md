...
**Last updated:** 2026-05-11 (Cron → edge-function auth switched to a shared `CRON_SECRET` bearer instead of `SUPABASE_SERVICE_ROLE_KEY`. Vercel's service-role key is now the new `sb_secret_…` opaque format which isn't a JWT, so the platform's `verify_jwt` gate rejected the cron's call with UNAUTHORIZED_INVALID_JWT_FORMAT before our code could run. Robust fix: literal CRON_SECRET compare in the edge function dispatcher + `verify_jwt: false` so the platform passes through. Eliminates the dependency on Supabase key formats. Also: AI Co-pilot scaffolding (PR 1, #50) lays groundwork for the agentic phase — see entry below.)
