...
**Last updated:** 2026-05-15 (**Post plan auto-titling.** When an admin saves the first piece of copy on a still-untitled `PostPlanDetailView`, the `concept` is auto-filled from a derived one-line title (first non-empty line, emoji/punctuation stripped, sentence-first preferred, truncated at word boundary to 60 chars). Fires once per plan (gated by empty concept + zero saved copy across platforms). A dismissible sparkles callout with Undo surfaces what happened. Pure client-side string transform, no AI call, no schema change. Lives entirely in `web/src/components/PostPlanDetailView.jsx`.)
