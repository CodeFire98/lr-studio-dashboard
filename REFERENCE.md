...
**Last updated:** 2026-06-16 (**Daily-digest (next-day post reminder) emails fixed — Resend 429 rate-limit, same bug usage-digest already had.** RCA of "reminder emails stopped after June 11": NOT code (zero commits Jun 8–16) and NOT the cron/auth/Vault (all healthy — `daily_digest_log` shows the job firing nightly at 18:00 IST for every brand). The audit log showed brands with posts-tomorrow failing at the LAST hop: `Bamboo Bear · sent 0 · failed 4 · send_failed: Resend 429: ... 2 requests per second`. Root cause: `handleDailyDigest` in `supabase/functions/send-email/index.ts` sent all of a brand's recipients via a single `callResendBatch` (`/emails/batch`) call — and Resend counts EACH email in a batch toward the 2-req/sec cap, so a 4-recipient brand 429s as one batch. This is the identical failure the **service-usage-daily** template already fixed in PR #127 (1100ms spacer + per-recipient `callResend` with 429 retry) — the daily-digest path just never got that treatment. The "June 11" cutoff = when Bamboo Bear's recipient count / posts-scheduled-tomorrow started tripping it every night (before that, sends that happened to stay ≤2/sec slipped through). Fix: replaced the `callResendBatch` call in `handleDailyDigest` with the proven one-at-a-time loop — `SPACER_MS = 1100` between recipients + `callResend` (built-in 1500ms 429 retry). Brand recipient counts are small, so the added latency is harmless and stays well within the function timeout. Cross-brand bursts (two brands qualifying the same night) are covered by callResend's retry. **The `send-email` edge function must be REDEPLOYED to Supabase** for this to take effect (it's a Deno edge function, not shipped by the Vercel merge). Verified: esbuild clean. Files: `supabase/functions/send-email/index.ts`. Sections touched: Recent changes log; `Last updated`. (Sibling fix: the service-usage-daily / usage-digest 429 fix in PR #127.))
