...
**Last updated:** 2026-05-14 (**AI Co-pilot marketing skills — PR 2 of N (skill registry + load_skill / load_skill_reference tools)**. 7 marketing playbooks copied from the [marketingskills](https://github.com/coreyhaines31/marketingskills) repo into `web/src/data/skills/` — social-content, content-strategy, copywriting, copy-editing, marketing-psychology, marketing-ideas, launch-strategy. ~3,800 lines of playbook material total. New `web/src/lib/skillRegistry.js` exposes `loadSkill(slug)` and `loadSkillReference(slug, ref)` for the API route. Two new tools on `/api/ai/chat` — `load_skill` and `load_skill_reference` — let the model pull a playbook on demand when the admin's request matches one (e.g. "brainstorm a launch" → load launch-strategy; "polish this caption" → load copy-editing). Skill menu (~650 cached tokens) injected into the system prompt so the model can route without loading; bodies (~1,500-5,000 tokens each) load only when relevant. vercel.json updated with `functions["api/ai/chat.ts"].includeFiles` to ship the .md files with the serverless bundle. Multi-candidate path resolution in the registry handles Vercel's ESM bundler placing modules in non-standard locations.)
...
- **vercel.json** — added `functions["api/ai/chat.ts"].includeFiles: "src/data/skills/**"` so the .md files ship with the chat function's serverless bundle. Without this, `fs.readFileSync` calls at runtime would 404 (Vercel only bundles JS files reachable via `import` by default).
- **Cost shape**: skill menu adds ~650 cached tokens to every chat turn (~$0.0002 read). Each loaded skill body is 1,500-5,000 tokens of tool-result content that rides in conversation context until the chat ends. Conservatively, +$0.01-0.04 per "smart" conversation that loads 1-2 skills; $0 for simple turns that don't load anything. Per user's directive, no per-conversation skill cap — quality over cost for now.
