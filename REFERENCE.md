...
**Last updated:** 2026-05-11 (AI Co-pilot scaffolding — PR 1 of the AI-native phase. Adds `brand_kit_notes` table for free-form admin "memory" annotations, plus `post_plans.ai_generated` + `post_plans.ai_draft_payload` columns to mark AI-proposed plans. Brand-context compiler (`web/src/lib/brandContext.js`) assembles a single cached system-prompt blob from `brand_kits` + `brand_kit_notes` + recent approved plans for Claude calls. Pure foundation — no UI, no API route, nothing in the app reads these yet. PR 2 lands the `/api/ai/chat` Vercel route + sidebar Co-pilot panel behind a brand-id whitelist env var.)
...
- **Migration `0038_ai_copilot_scaffolding`** — two pieces:
...
- **Sections touched**: Recent changes log; `Last updated`; Glossary (new entries: AI Co-pilot, brand_kit_notes, AI draft, brand context); §6 Data model (new `brand_kit_notes` table + new `post_plans` columns); §6 Migrations list (0038 row); §13 Known decisions (new entry: compiler-as-pure-function for cache stability; new entry: notes-as-table-not-jsonb); §14 Pending work (PR 2 scope: `/api/ai/chat` route + sidebar Co-pilot panel + brand-id whitelist).
...
| **brand_kit_notes** | Free-form admin annotations on a brand — "remember that the founder hates the word 'authentic'", "no holiday content until Oct 15", "always tag @cofounder on milestone posts". The "memory" layer for the AI Co-pilot. Written by hand from the BrandKit UI (later PR) AND by the Co-pilot via a `write_brand_note` tool. `is_pinned=true` rows are always-true facts that ride along on every AI call; non-pinned rows are recent context that decays out of the window once we hit the ~20-most-recent cap. | `brand_kit_notes` table (migration 0038) |
