...
The Vercel route `web/api/slack/brand-message-notify.ts` verifies the bearer, re-fetches the message with service-role (two PostgREST hops — message + author + tagged plan via FK embeds, then `accounts` row for the brand display name + URL slug; brand identity lives on `accounts`, not `brand_kits`), assembles a Block Kit payload (header with brand name, author + IST timestamp context, blockquote body truncated at 600 chars, optional "On plan: <title>" chip, two action buttons: "Open conversation" deep-linking `/c/<slug>/conversations#msg-<uuid>` and "Open plan" when `tagged_post_plan_id` is set), POSTs to `SLACK_BRAND_MSG_WEBHOOK_URL`, then updates the log row with `delivered_at` / `slack_status` / `slack_error`.
