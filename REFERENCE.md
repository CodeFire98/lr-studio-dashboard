...
**Last updated:** 2026-05-14 (**Conversations PR 1 of 2 — data layer + sidebar plumbing**. New per-brand chat schema (`conversations` + `conversation_messages` + `message_attachments` + `conversation_views`) with RLS + realtime + a trigger that auto-provisions a conversation row when a brand account is created. Migration 0042 backfills one conversation per existing brand and copies every `post_plan_comments` row into `conversation_messages` with the plan auto-tagged. The legacy per-plan Conversation tab inside `PostPlanDetailView` is repointed at the new tables transparently — no UX change yet — and the calendar comment-count badge + per-plan unread dot read from the same source. New "Conversations" sidebar entry routes to a placeholder `ConversationsView` that marks the brand's conversation seen on mount; PR 2 replaces the placeholder with the real chat UI (top-level feed + composer + thread drawer).)
...
| **Conversations** *(unified per-brand chat)* | One ongoing chat per brand account. Brand sees a single thread with their agency; agency sees the same thread, scoped via BrandPicker. Replaces the old per-plan Conversation tab as the home for all back-and-forth. Optional `tagged_post_plan_id` per message renders a clickable plan-preview card inline. Slack-style threads via self-FK `parent_message_id` (UI lands in PR 2). | `conversations` + `conversation_messages` + `message_attachments` + `conversation_views` tables (migration 0042); sidebar entry "Conversations" with `chat` icon; route `"conversations"`, path `/c/:slug/conversations`. PR 1 ships data layer + placeholder view; PR 2 ships the chat UI. |
...
- **X engagement isn't tracked in MVP — no viable Apify actor.** Two preflight rounds against real public tweet URLs on 2026-05-12 evaluated 5 candidate actors. Three were broken or missing (`apidojo/tweet-scraper`, `tugkan/...-pay-per-result`, `apidojo/twitter-scraper-lite`); two charged for empty results (`kaitoeasyapi/...-cheapest` with explicit fine-print disclosure; the apidojo siblings ran in demo mode for unrecognized inputs). Considered (a) commit to one of the apidojo actors and fix the input shape iteratively, (b) pay for the official X API at \$200/mo Basic tier, (c) skip X entirely in MVP and show a permanent "not tracked" badge. Chose (c). Reasons: (i) the per-call-on-empty-results pricing pattern is uncapped cost risk — every deleted/protected/restricted tweet would still bill us, (ii) \$200/mo for X-only metrics is hard to justify when the rest of the feature lands at ~\$15/mo full-rollout, (iii) X engagement is the lowest-value of the three platforms for our agency context (most brand engagement happens on IG/LinkedIn). The X filter pill stays in Live Posts so users can still mark plans as posted to X and see the URL; the tile shows the post exists but doesn't pretend to track metrics. Trivially reversible later if a brand explicitly demands X analytics — just remove the badge and add an `scrapeX` function to the dispatch. **Lesson on Apify actor selection**: query Apify Store API (`/v2/store?search=…`) for actor names instead of guessing — both my PR-1 and my preflight-1 actor names were wrong end-to-end. The Store API is public and free; querying it before the dry-run would have saved a full credit-burning iteration.
...
- **Conversations PR 2 — chat UI** *(next-in-queue, promoted 2026-05-14 after PR 1 shipped data + sidebar plumbing)*: replace the placeholder `ConversationsView` with the real chat surface. Single-pane stacked feed of top-level `conversation_messages`, composer at the bottom (text + 📎 attach + 🔖 tag-a-plan dropdown), Slack-style thread drawer that slides in from the right when you click "Reply in thread" on any message. Plan-tagging renders as a small clickable card inside the bubble with concept + status pill. Drop the per-plan Conversation tab entirely from `PostPlanDetailView` and replace it with a small "💬 Discuss this plan" button that deep-links to `/c/:slug/conversations` with the plan pre-tagged in the composer. Schema is ready (parent_message_id + tagged_post_plan_id + message_attachments table all exist from 0042) — no migration needed. Roll attachments + link previews into the same PR; defer @mentions, DMs, and email triggers to PR 3.
- **Conversations PR 3 — attachments + link previews** *(after PR 2)*: hook up the `message_attachments` table to a new `conversation-attachments` storage bucket (mirror of `post-plan-attachments`'s RLS shape, scoped via `<accountId>/<conversationId>/<messageId>/...` path). Drag-drop + 📎 button. Image / video previews inline; PDFs/zips as file chips; pasted URLs auto-rendered as link cards via Firecrawl `/scrape` `metadata` (cached server-side).
- **Conversations follow-ups (deferred / long-tail)**: drop the legacy `post_plan_comments` table once the new chat UI has baked for a week (see §13 entry). Add `@mention` typeahead + Resend Tier 2 emails on mention-received. Add per-thread unread tracking (today the sidebar badge counts top-level only — once threads are heavily used, adding a per-thread last-seen mechanism keeps the dot tight). DMs and group chats remain explicitly out of scope; revisit only if non-technical brand users start asking.
