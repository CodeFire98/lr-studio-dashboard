...
**Last updated:** 2026-06-17 (**send-email: aggressive 429 retry (Retry-After + exp backoff) — and a correction to the batch RCA.** Follow-up after PR #170's per-recipient daily-digest fix was merged but the daily-digest STILL 429'd on the 2026-06-17 18:00 cron (Bamboo Bear 4 recipients failed) while an isolated 4-recipient curl returned `ok:true sent:4` in **0.637s**. Two findings: (1) **The redeploy of `send-email` hadn't taken** — 0.6s is impossible for the spaced code (4 recipients × 1100ms = ≥3.3s), so the live function was still the OLD batch version; PR #170 was never actually deployed. The `time <curl>` duration (~3.3s vs ~0.6s) is now the deploy litmus test — DON'T trust `ok:true`. (2) **Correction:** a 4-recipient `/emails/batch` call succeeds in isolation (one request, 0.6s) — so Resend does NOT charge per batch-item as the PR #127/#170 comments assumed. The real cause of the cron 429 is **contention at 18:00 IST on the shared Resend key** (other traffic eats the 2/sec budget; an ad-hoc curl at a random minute doesn't collide). Fix: hardened `callResend`'s retry from 1 attempt @1500ms to **up to 4 attempts honoring the `Retry-After` header, else exponential backoff (1s/2s/4s)** — rides out a multi-second contention burst instead of hard-failing the brand. Durable fix still recommended: a **dedicated Resend key / higher plan** for this app (the 2/sec ceiling is the binding constraint; it's bitten usage-digest + daily-digest). NOTE: `send-email` is a Deno edge function — **must be redeployed to Supabase from current `main`** (verify via the 3.3s timing). Verified: esbuild clean. Files: `supabase/functions/send-email/index.ts`. Sections touched: Recent changes log; `Last updated`.)

**Previous update:** 2026-06-16 (**Daily-digest (next-day post reminder) emails fixed — Resend 429 rate-limit, same bug usage-digest already had.** RCA of "reminder emails stopped after June 11": NOT code (zero commits Jun 8–16) and NOT the cron/auth/Vault (all healthy — `daily_digest_log` shows the job firing nightly at 18:00 IST for every brand). The audit log showed brands with posts-tomorrow failing at the LAST hop: `Bamboo Bear · sent 0 · failed 4 · send_failed: Resend 429: ... 2 requests per second`. Root cause: `handleDailyDigest` in `supabase/functions/send-email/index.ts` sent all of a brand's recipients via a single `callResendBatch` (`/emails/batch`) call — and Resend counts EACH email in a batch toward the 2-req/sec cap, so a 4-recipient brand 429s as one batch. This is the identical failure the **service-usage-daily** template already fixed in PR #127 (1100ms spacer + per-recipient `callResend` with 429 retry) — the daily-digest path just never got that treatment. The "June 11" cutoff = when Bamboo Bear's recipient count / posts-scheduled-tomorrow started tripping it every night (before that, sends that happened to stay ≤2/sec slipped through). Fix: replaced the `callResendBatch` call in `handleDailyDigest` with the proven one-at-a-time loop — `SPACER_MS = 1100` between recipients + `callResend` (built-in 1500ms 429 retry). Brand recipient counts are small, so the added latency is harmless and stays well within the function timeout. Cross-brand bursts (two brands qualifying the same night) are covered by callResend's retry. **The `send-email` edge function must be REDEPLOYED to Supabase** for this to take effect (it's a Deno edge function, not shipped by the Vercel merge). Verified: esbuild clean. Files: `supabase/functions/send-email/index.ts`. Sections touched: Recent changes log; `Last updated`. (Sibling fix: the service-usage-daily / usage-digest 429 fix in PR #127.))

**Previous update:** 2026-06-16 (**Engagement: a degraded/empty scrape no longer clobbers a good embed (the "Unknown author" follow-up).** After the last-good METRICS fallback shipped, LinkedIn numbers reappeared but the post EMBEDS still showed "Unknown author" / no caption / no image, footer "Showing last good figures (Nd ago) · latest refresh incomplete". RCA: `persistScrapeResult` (both the Vercel `web/api/_shared/scraper-lib.ts` AND the daily-cron `supabase/functions/engagement-refresh/index.ts`) upserted `post_embed_cache` on `if (result.ok && result.embed)` — but a `partial` result is `ok:true` with an embed object whose author/caption/media are all null, so a degraded/empty scrape OVERWROTE the previously-good embed row with nulls. Metrics had a last-good fallback (snapshots are append-only history, read-side picks the last row with metrics); the embed cache is a single 1:1 upsert row with no history, so once clobbered the good author/caption/media were gone — hence "Unknown author" with the numbers still showing via the metrics fallback. Confirmed the actor is healthy (ran `apimaestro/linkedin-post-detail` live against the failing Jun-11 Bamboo Bear URL: `total_reactions:17`, author "Bamboo Bear", caption + media all present), so the empty results are coming from the production scrape path, not the actor — strongly implicating the daily cron still running pre-#166 code (its empty `partial`s clobbered the embeds; a fresh apimaestro scrape repopulates them). Fix: added an `embedHasContent` guard in BOTH `persistScrapeResult` impls — the embed upsert now runs only when the scrape actually returned author/handle/avatar/caption/media (else it's skipped, preserving the last-good embed). A genuinely good scrape always repopulates it. Restoring an already-clobbered embed needs one successful scrape ("Refresh now" via the Vercel path, or the next cron once the edge function is confirmed on the new actor). **The edge function must be redeployed** to pick this up (Vercel path ships on merge). Verified: esbuild clean on both files. Files: `web/api/_shared/scraper-lib.ts`, `supabase/functions/engagement-refresh/index.ts`. Sections touched: Recent changes log; `Last updated`.)

**Previous update:** 2026-06-16 (**Engagement: a failed/empty scrape no longer blanks out last-good metrics (cards + summary).** RCA of "LinkedIn metrics worked for days then went blank again": the daily cron's edge function was running pre-#166 code (`engagement-refresh` was last deployed ~2026-05-31 — before the 06-03 actor swap), so every nightly run scraped LinkedIn with the dead `supreme_coder` actor and wrote a fresh **null-metric `partial`** snapshot. Both read paths — `loadLatestEngagementSnapshots` (cards) and `latestSnapshotAtOrBefore` (summary `aggregateAtTime`/`aggregatePerMetricAtTime`) — display the **absolute latest** snapshot per publication, so each bad nightly scrape *overwrote the displayed good numbers*, blanking the cards ("Unknown author", null metrics, `partial`) and the per-platform summary row ("No metrics in this window"). Verified the actor itself is **healthy** — ran `apimaestro/linkedin-post-detail` 6× live against a real Bamboo Bear company-post URL: returns `total_reactions:17`/author/caption/media every time (so NO actor swap needed; my initial "actor degraded again" read was wrong). Fix (pure client, read-side): a failed refresh means "couldn't read it this time", not "engagement is now zero" — so the UI now falls back to the most recent snapshot that **actually carried metrics**. New `snapshotHasMetrics(s)` (true when any count is numeric — a genuine 0-engagement post still qualifies; only all-null failed/blocked/empty scrapes don't). `loadLatestEngagementSnapshots` now surfaces the LATEST snapshot for refresh status/error/timestamp but overlays metric fields from the last-good snapshot, flagged `metricsStale` + `metricsFetchedAt`. New `latestSnapshotWithMetricsAtOrBefore` replaces `latestSnapshotAtOrBefore` in both summary aggregators (skips metric-less snapshots, respects the as-of cutoff). `LivePostsView` `refreshFooter` now says "Showing last good figures (Nd ago) · latest refresh failed" instead of implying no data. Net effect: a transient actor hiccup or a stale-cron run can never again wipe the displayed numbers; data self-heals on the next good scrape. **Operator note:** posts that NEVER had a good scrape (e.g. a brand-new post only ever hit by the old cron) still show blank until one successful scrape — click "Refresh now" (Vercel path, working actor) or wait for the next 06:00 IST cron (now redeployed with the new actor). Verified: esbuild clean on db.js + LivePostsView.jsx; 10/10 last-good logic cases pass (latest-good, latest-failed-overlay, no-good-history, zero-engagement-is-real, aggregator skip + cutoff). Files: `web/src/lib/db.js`, `web/src/components/LivePostsView.jsx`. **Also in this change — unblocked the prod Vercel deploy (Hobby 12-function limit):** the prod project `lr-studio-dashboard-3kkp` is on Vercel's Hobby plan, which caps **Functions Created per Deployment at 12** (per-deployment, NOT a per-account pool; Pro = ∞). The repo emitted **14** because two helper LIBRARIES under `web/api/**` were being auto-detected as Serverless Functions (every non-`_`-prefixed file under `web/api` becomes a function): `web/api/ai/auth-lib.ts` (imported by chat/copy/image/suggestions) and `web/api/engagement/scraper-lib.ts` (imported by refresh). Moved both into the already-excluded `web/api/_shared/` dir (underscore-prefixed dirs/files are not treated as functions — same trick that already hides `_shared/usage.ts` + `_shared/textNormalize.ts`), updating the 5 importers' relative paths (`./auth-lib.js`→`../_shared/auth-lib.js`, `./scraper-lib.js`→`../_shared/scraper-lib.js`; `.js` extension kept per the Node24-ESM rule). Net: 14→12 detected functions, deploys on Hobby. **This leaves zero headroom** — the next new `web/api/**` route will re-break prod; the durable fix is upgrading the project to a Vercel Pro team (also lifts the sub-daily-cron limit). Files: `web/api/_shared/auth-lib.ts` (moved), `web/api/_shared/scraper-lib.ts` (moved), `web/api/ai/{chat,copy,image,suggestions}.ts`, `web/api/engagement/refresh.ts`. Sections touched: Recent changes log; `Last updated`; deployment/function-count note.)

**Previous update:** 2026-06-08 (**Engagement summary strip now live-updates after a snapshot lands (LinkedIn "No metrics in this window" follow-up to the 2026-06-03 actor fix).** After the LinkedIn actor swap restored card-level metrics, the per-platform **Engagement Summary** row for LinkedIn still showed "No metrics in this window" until a full page reload. RCA: the post tiles in `LivePostsView` repaint live via the realtime snapshot subscription (`subscribeToAllEngagementSnapshots` → `snapshotsByPubId`), but `LivePostsSummary` is a one-shot read (`loadEngagementSummaryForBrand`) with **no** realtime subscription of its own — its fetch effect keyed only on `[accountId, period]`. So a "Refresh now" (or daily cron) that wrote a fresh LinkedIn snapshot updated the cards but left the summary aggregating the pre-fix, null-metric snapshots. Fix (pure client): `LivePostsView` now owns an `engagementVersion` counter bumped inside the realtime snapshot handler, passed to `<LivePostsSummary refreshSignal={engagementVersion} />`; the summary's fetch effect adds `refreshSignal` to its deps so it re-aggregates whenever a snapshot lands — same mechanism the tiles already use. No schema/migration/env/server change. Verified: esbuild syntax-clean on both files; user-confirmed LinkedIn row populates. Files: `web/src/components/LivePostsView.jsx`, `web/src/components/LivePostsSummary.jsx`. Sections touched: Recent changes log; `Last updated`.)

**Previous update:** 2026-06-03 (**Engagement: LinkedIn scraper fixed — actor swap + error-classification (RCA fix a+b).** RCA of "Live Posts → LinkedIn shows No metrics in this window": the third-party Apify actor `supreme_coder/linkedin-post` broke ~2026-06-01 (LinkedIn markup change) and now returns HTTP 200 with a dataset item shaped `{error:"Failed to scrape post: Cannot read properties of undefined (reading '0')", inputUrl}` instead of post data — for every LinkedIn URL. LinkedIn was healthy through 2026-05-31 (145→188 reactions etc.). Contributing bug: both `scrapeLinkedIn` implementations only treated an EMPTY dataset (`!item`) as failure, so the 200-with-error-item fell through, read `numLikes/authorName/text` as null, built a null-metric snapshot, and labelled it `partial` — which the UI renders as "No metrics in this window" / "Unknown author" AND which hid the outage from monitoring (snapshot overview showed `failed:0, blocked:0`; the breakage all sat in `partial`). Fix a: both `scrapeLinkedIn` (the Vercel on-demand path `web/api/engagement/scraper-lib.ts` AND the daily-cron Deno edge function `supabase/functions/engagement-refresh/index.ts`) now detect an item with a non-empty `error` field and return `status:"failed"` with the actor message surfaced, so the failure is alertable and distinguishable from a genuinely zero-engagement post. Fix b (the actual restore): swapped the dead `supreme_coder/linkedin-post` → **`apimaestro/linkedin-post-detail`** (no-cookies, accepts a single post URL or activity id) in BOTH `scrapeLinkedIn` implementations + all three `APIFY_USD_PER_SCRAPE` cost maps ($5/1k results). New input is `{post_urls:[cleanUrl]}` (query string stripped); the nested response maps `stats.total_reactions`→like/reaction count, `stats.comments`→comments, `stats.shares`→shares, `author.name`→display name, handle parsed from `author.profile_url` (`/in/<h>` or `/company/<h>/posts`), `author.profile_picture`→avatar, `post.text`→caption, `media[].url`→media, `post.created_at.timestamp`→posted_at. **Verified live 2026-06-03** against Bamboo Bear's 3 real LinkedIn URLs: 13/0/0, 193/14/2 (matches our last good scrape of 188/14/2 + growth), 50/2/1 — handles `drinkbamboobear` / `theshrutimishra` resolved correctly. Secondary finding (latent, NOT fixed): daily cron caps ~20 scrapes/run but Bamboo Bear has 22 live pubs, so LinkedIn lags ~1 day in rotation; raise/scale the cap as volume grows. **The edge function must be redeployed** for the cron to use the new actor (the Vercel on-demand path ships on merge). No prod secret change — the new actor reuses the existing `APIFY_API_TOKEN`. Verified: `tsc` clean on scraper-lib.ts + refresh.ts; mapping validated against the live actor output. Files: `web/api/engagement/scraper-lib.ts`, `supabase/functions/engagement-refresh/index.ts`, `web/api/_shared/usage.ts`. Sections touched: Recent changes log; `Last updated`; §9/§10 engagement scraper + edge functions.)
