...
**New cron route — [`web/api/engagement/refresh-cron.ts`](web/api/engagement/refresh-cron.ts).** Vercel Cron `0 */6 * * *` (every 6h). Auth via `Authorization: Bearer ${CRON_SECRET}` — same pattern as `daily-digest`. Algorithm:
