...
**Last updated:** 2026-05-21 (**LinkAI PR A — sidebar reorder + new full-page `/c/:slug/linkai` surface + mermaid snag fix.** First of four planned PRs that promote the AI Co-pilot from a right-side drawer into a first-class sidebar destination (with later PRs B/C/D renaming + adding history-rail + retiring the drawer).
...
**(2) New `/c/:slug/linkai` full-page surface.** `linkai` added to `SIMPLE_VIEWS`, `BRAND_SCOPED_VIEWS`, `inBrandRoutes`, the bare-path `parsePathToRoute` block, and the topbar breadcrumb. `renderView` mounts `<CopilotPanel variant="page" />` inside a `<div className="view"><div className="view-inner linkai-page-inner">` shell with a `Suspense` fallback (CopilotPanel is lazy-imported per the existing v2 migration). `CopilotPanel` accepts a new `variant` prop — `'panel'` (default, unchanged) or `'page'`. In page variant: outer class adds `.copilot-panel--page`, close button is suppressed, header label says "LinkAI" instead of "Co-pilot", placeholder says "Message LinkAI…", and the empty-state welcome renders a large serif hero ("Tell me what you want to make for {brandName}. I can ideate, research, draft posts, and plan your calendar — just ask.") instead of the panel's compact welcome card. The right-side drawer behaviour from the topbar "✨ Co-pilot" trigger stays untouched (both surfaces co-exist; PR D retires the drawer).
...
**(4) New CSS** (~110 lines added to `app.css`) for `.copilot-panel--page` (inline positioning instead of fixed; full-width; flex-fill height), `.copilot-welcome--page` (serif hero), `.copilot-welcome-hero`, `.copilot-suggestions` grid layout in page variant, and `.nav-sep`. Sidebar guest-mode is unchanged (LinkAI is only in `buildBrandNav`, not `GUEST_NAV`).
