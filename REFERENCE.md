...
**Last updated:** 2026-05-02 (Send-update batch email + Duplicate gated to agency-only)
...
- **Agency-update fans out one Resend call per recipient, not a single multi-`to`/BCC send.** Resend supports passing multiple addresses, but that either reveals the full recipient list to everyone (`to: [...]`) or sends a single envelope with no per-recipient personalization (`bcc: [...]`). Considered both. Chose N independent calls so (a) members never see other members' addresses, matching standard transactional-email privacy; (b) per-recipient failure is granular — we return `{ sent, ids, failed }` so the modal can tell the agency exactly which addresses bounced. N is small (typical brand workspace = 1–5 members) so the latency cost is negligible.
