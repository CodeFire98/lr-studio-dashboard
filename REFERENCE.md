...
**Last updated:** 2026-05-08 (Daily 6pm-IST digest of tomorrow's posts to brand members — Vercel Cron + new `daily-digest` template on `send-email`; per-brand toggle via `accounts.daily_reminder_enabled` migration 0037. Earlier today: sidebar "Got ideas?" → "Idea dump" relabel + invite-redemption pre-flight email check.)
