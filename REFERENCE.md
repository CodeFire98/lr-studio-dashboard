...
**Last updated:** 2026-06-02 (**Fix: LinkAI image attachments 413'd silently — downscale before send.** Bug: sending a LinkAI chat message with image attachment(s) showed "thinking…" for a split second then the bubble vanished with no response, while text-only requests (plans, ideas) worked fine. Root cause confirmed via Vercel runtime logs (no `/api/ai/chat` function log at all for image requests → the request never reached the function): attachments are sent as base64 data URLs **inside the JSON request body**, and full-size photos (a 5 MB photo ≈ 6.7 MB base64; two of them) exceed Vercel's ~4.5 MB serverless body limit → **413 at the edge before the function runs**. The AI SDK's `useChat` rolls back the streamed message on the failed fetch, and the existing error banner is gated on `streamingActiveConv` (which a pre-invocation 413 never sets), so it surfaced as a silent disappear. Fix: new shared util `web/src/lib/imageDownscale.js` (`downscaleImageToDataUrl` — canvas resize to ~1568px long edge, JPEG q0.85, white matte for transparent PNGs; ~1568px is Claude vision's own downsample bound so no useful detail is lost) wired into `LinkAIPanel.jsx`'s `addFiles` so every attachment is shrunk to ~150-400 KB (from multiple MB) before it enters state / the body. Source-size guard raised 5 MB → 25 MB (now just a decode-sanity cap; downscaling handles payload), attachments re-encoded so `mediaType` is `image/jpeg`. NOTE: this is a PROD bug independent of the in-flight image-prompting PRs (#161/#162) — it ships on its own branch `fix/linkai-image-downscale`. The same downscale approach is mirrored in those PRs' post-plan image panel; when they land, `AIImagePromptPanel`'s inline copy should be refactored to import this shared util. Pure client — no schema/migration/env/server change. Verified: `vite build` clean. Files: `web/src/lib/imageDownscale.js` (new), `web/src/components/LinkAIPanel.jsx`. Sections touched: Recent changes log; `Last updated`.)
...
**Reference-image-aware (forward-compat for product-image upload)**: the house-style block tells the model that if reference image(s) are attached it must describe the product's shape / label / colour / proportions from what it SEES (never invent), and otherwise close with the reference-line instruction. PR 1 attaches no images (text-only); the planned product-image-upload PR feeds the real product shot to Claude via vision with no prompt rewrite needed.
