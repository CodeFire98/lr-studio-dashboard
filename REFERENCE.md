...
**Last updated:** 2026-05-26 (**Paste images directly into the LinkAI composer AND the Conversations composer with Ctrl/Cmd+V.** Third entry point for image attachments on both per-brand chat surfaces, alongside the existing paperclip button (and, on LinkAI, drag-drop). Each surface wires an `onPaste` handler on its composer textarea, scoped to fire only when the composer has focus so it doesn't surprise users pasting anywhere else on the page.
