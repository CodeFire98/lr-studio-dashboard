...
| `brand_kits` | Brand's design + voice profile | one per account; ~60 columns including `palette`, `fonts`, `tagline`, `mission`, `voice_tags`, `logos`, `enrichment_status`, `enriched_at`. **Brand System v1 (2026-05-24)** adds two JSONB columns: `claim_guardrails` (structured never-use / always-pair / approved-qualifier rules) and `channel_voice` (per-channel voice prescription with `global` + IG/LinkedIn/X/WhatsApp sub-objects). Both default `{}`; consumed by `brandContext.js` and injected into every AI-context blob. **Image-prompting PR 2 (2026-06-01)** adds `product_reference_images jsonb default '[]'` (migration 0063): array of `{id, path, filename, mimeType, sizeBytes, addedAt}` pointing at the private `brand-product-images` bucket; `/api/ai/image` feeds up to 3 (most recent) to Claude as vision input. | Members + agency | Members + agency |
...
| `brand-product-images` | Brand product reference photos for AI image prompting | Private (signed URLs) | Read + write/delete: agency or account members; path-scoped `<accountId>/<ts>_<filename>` via `brand_product_image_account_id(name)` helper (migration 0063). `/api/ai/image` reads bytes via service-role for Claude vision |
...
Most recent: `0063_brand_kit_product_images.sql`.
...
- `0063_brand_kit_product_images` — Image-prompting PR 2. Adds `brand_kits.product_reference_images jsonb default '[]'` (array of `{id, path, filename, mimeType, sizeBytes, addedAt}`) + private Storage bucket `brand-product-images` (path `<accountId>/<ts>_<filename>`, helper `brand_product_image_account_id`). RLS: read + write/delete by agency or path-scoped account members. `/api/ai/image` feeds up to 3 (most recent) to Claude as vision input. Applied to prod via MCP `apply_migration` (tracked name `brand_kit_product_images`).
...
**Reference-image-aware (PR 2 — product-image vision, shipped 2026-06-01)**: the house-style block tells the model that if reference image(s) are attached it must describe the product's shape / label / colour / proportions from what it SEES (never invent), and otherwise close with the reference-line instruction. The route now loads `brand_kits.product_reference_images`, downloads up to **3 (most recent)** via the service-role storage client (`loadProductImages` → `brand-product-images` private bucket → `Uint8Array`), and sends them as **AI SDK image parts** in the user message (multimodal: image parts first, then the text) on **both** modes. When images are present an explicit note is appended to the user text ("N product reference images attached — they are the ACTUAL product…"). Media type is resolved from the stored `mimeType` (fallback: filename extension), restricted to Anthropic's accepted set (jpeg/png/gif/webp); anything else is dropped. Best-effort throughout — a failed download or empty library degrades to text-only, identical to PR 1. Brands manage the library from **two** surfaces that both write the same `brand_kits.product_reference_images` array: the `ProductImagesCard` in `BrandKitView`, and an inline `ProductRefStrip` at the top of the AI image-prompt panel's compose steps (`AIImagePromptPanel.jsx`) so an admin can upload a product shot mid-prompt without leaving the panel (uploads via `addBrandProductImage`, thumbnails via `getBrandProductImageUrl`, list via `listBrandProductImages`; the latest 3 carry an "AI" badge). Image count is logged as `images=N` on the `[image] usage…` line and `product_images` in `service_usage_log.meta`. Images are sent **uncached** for now (they ride the per-call user message; caching them would consume the 4th cache breakpoint — noted as a future optimization).
