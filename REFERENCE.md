...
- **Vercel excludes `api/_*.ts` from function bundles — for routes AND helpers.** Discovered 2026-05-14 during the X integration deploy. The old understanding (per `feedback_vercel_underscore_prefix.md`) was "don't name a ROUTE with leading underscore; helpers are fine because the file still exists on disk for sibling imports." Turns out helpers are NOT fine. Vercel's build strips leading-underscore files from the deployed function artifact entirely. A route that imports from `./_shared` throws `ERR_MODULE_NOT_FOUND` at runtime (`Cannot find module '/var/task/.../_shared'`). The on-demand `/api/engagement/refresh` had been working since PR 7 on Node 22, then started 500ing under Node 24 (Vercel's new default) on the X-integration deploy with the same `_shared.ts` import that had been fine before. Fix: rename `_shared.ts` → `scraper-lib.ts` (or any name without a leading underscore) and update the two import sites. Memory file `feedback_vercel_underscore_prefix.md` updated to capture both failure modes.
