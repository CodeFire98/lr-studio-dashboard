...
**Planned:**
- **PR 3** — touch polish: `@media (hover: hover)` guards on hover-only affordances that don't have natural fallbacks, drag-drop hint copy hidden on coarse-pointer, ConversationsView Composer + PostPlanDetailView attachment uploaders gain camera capture buttons (same pattern as IdeateView), LinkAI history rail becomes a top-sheet drawer toggled from the chat header (PR 2 just hides it).
- **PR 4** — final polish: bottom-sheet drag-to-dismiss, iOS PWA meta tags, landscape orientation, unify `.modal-scrim` and `.login-modal-backdrop` classnames.
