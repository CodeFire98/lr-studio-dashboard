...
**Last updated:** 2026-05-14 (**Conversations PR 2 of 2 — chat UI shipped**. Replaces the PR 1 placeholder with a real Slack-style chat at `/c/:slug/conversations`: full-pane stacked feed of top-level messages, composer with optional 🔖 tag-a-plan dropdown, message bubbles render an inline clickable plan-chip card when tagged, "Reply in thread" opens a right-side drawer with the parent pinned + replies + its own composer (Esc / scrim closes; full-screen overlay on mobile). Realtime: new messages and new replies appear without refresh. The per-plan **Conversation tab is gone** from `PostPlanDetailView` — replaced by a slim "💬 Discuss this plan" button in the actions row that deep-links to `/conversations?plan=<short-id>` with the plan pre-tagged in the composer. The "needs revision needs a reason" workflow that used to force-switch to that tab now opens an inline expanding panel right under the actions row instead. Attachments + @mentions remain deferred to PR 3 / long-tail. Schema unchanged from PR 1 — pure frontend PR.)
...
| **Conversations** *(unified per-brand chat)* | One ongoing chat per brand account at `/c/:slug/conversations`. Brand sees a single thread with their agency; agency sees the same thread, scoped via BrandPicker. Full-pane stacked feed + composer with optional 🔖 tag-a-plan dropdown + Slack-style thread drawer that slides in from the right when you "Reply in thread". Replaces the old per-plan Conversation tab as the home for all back-and-forth. Optional `tagged_post_plan_id` per message renders a clickable plan-preview card inline. Composer sends with ⌘↩; plain Enter inserts a newline. | `conversations` + `conversation_messages` + `message_attachments` + `conversation_views` tables (migration 0042); sidebar entry "Conversations" with `chat` icon; route `"conversations"`, path `/c/:slug/conversations`; deep-link `?plan=<short>` pre-fills composer tag. UI in [ConversationsView.jsx](web/src/components/ConversationsView.jsx); CSS namespaced under `.conv-*` in [app.css](web/src/styles/app.css). Attachments + @mentions deferred to PR 3 / long-tail. |
...
- **Conversations PR 3 — attachments + link previews** *(next-in-queue, promoted 2026-05-14 after the chat UI shipped)*: hook up the `message_attachments` table (already on disk from migration 0042) to a new `conversation-attachments` storage bucket (mirror of `post-plan-attachments`'s RLS shape, scoped via `<accountId>/<conversationId>/<messageId>/...` path). Drag-drop onto the composer + a 📎 icon-button next to the existing 🔖 tag-a-plan affordance. Image / video previews render inline in the bubble; PDFs/zips render as file chips with a download click-through; pasted URLs auto-rendered as link cards (Firecrawl `/scrape` `metadata` fetch on the server, cached on `message_attachments.kind='link'`). Reuse the existing image-dimension validator at upload time.
- **Conversations follow-ups (deferred / long-tail)**:
  - Drop the legacy `post_plan_comments` table once the new chat UI has baked for a week (see §13 entry on rollback safety).
  - Add `@mention` typeahead inside the composer + Resend Tier 2 emails on mention-received and DM-received (DMs themselves stay out of scope — see below).
  - Per-thread unread tracking — today the sidebar badge counts top-level only. Once threads are heavily used, adding a per-(user, thread-parent) last-seen mechanism keeps thread unread dots accurate without inflating the brand badge.
  ...
  - Emoji reactions on messages — low-effort, high-signal addition once the rest is stable.
