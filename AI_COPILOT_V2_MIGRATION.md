...
| 1c | Server: `/api/ai/image` → `streamObject` (ideas mode, Zod schema replaces lenient JSON parser) + `streamText` (prompt mode). Also DELETES `web/api/ai/cache-poc.ts` (PoC retired). | 🟡 open | [#65](https://github.com/CodeFire98/lr-studio-dashboard/pull/65) |
