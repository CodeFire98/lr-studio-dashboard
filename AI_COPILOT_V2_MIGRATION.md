...
- **2026-05-12** (Phase 3 — suggestion chips): `CopilotPanel.jsx`'s hardcoded `EMPTY_SUGGESTIONS` replaced with brand-aware chips templated from `recentApprovedPlans` + `brand_kits.product_categories` via new `loadCopilotSuggestions({ accountId })` helper in db.js. Client-side, no AI call, no new route, no cost. Chips upgrade in place from the v1 generic fallback to the brand-aware set after the Supabase round-trip.
